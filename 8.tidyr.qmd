---
title: "Lección 8: Manipulación de tablas"
author: "Tripp Valdez MA"
---

# Tidy data

En la lección XX aprendimos un poco sobre la filosofía Tidy y como se aplica en la construcción y análisis de datos tabulados.

Es posible representar los mismo datos en diversas maneras. Por ejemploe, a continuación se muetra la misma información organizada de manera diferente. Cada set de datos muestra el mismo valor de cuatro variables: `country`, `year`, `population` `cases`, pero cada set esta organizado de manera diferente
...
La `tabla3` muestra una situación muy común en la que las columnas no son nombres de variables; es decir, las columnas `1999` y `2000`representan valores de la variable `año`, y los valores en las columas `1999`y `2000`representan valores de la variable `casos`. Cada fila representa dos observaciones y no una.
...
2.  El nombre de la variable donde se moveran las columnas. En este caso sería `year`
...
Hasta ahora hemos examinado uniones que modifican la primera tabla agregando columnas de la segunda tabla. A continuación, están las uniones de filtrado. Estas funcionan de manera ligeramente diferente en el sentido de que las columnas de la primera tabla permanecen sin cambios y no se agregan nuevas. La unión se utiliza más como una forma de filtrar datos, es decir, eliminar filas según cómo se relacionen con otra tabla.
...
Para modificar esta columna podemos utilizar la función `separate()`. Esta función separa una columna en multiples columnas a partir de un caracter.
...
Por default, `separate()` devuelve variables del mismo tipo que la original, que en este caso son caracteres. En este ejemplo en particular, este comportamiento no es el mas conveniente, ya que en realidad se tratan de variables numericas. Para evitar este comportamiento se puede usar el parametro `convert = TRUE`
...
De igual manera, `separate()` elimina la columna que estamos separando pero si queremos conservarla, podemos utilizar el parametro `remove = FALSE` como se muestra en el siguiente ejemplo:

```{r}
table3 |> 
  separate(country, into = c("code"), sep = 3, remove = FALSE)
```

La función `unite()` tiene el comportamiento opuesto y permite unir dos variables ubicadas en dos columnas en una sola. Por ejempo, si queremos convertir `table1` a un formato similar a `table3`
...
Hay ocaciones en las que una variable tiene varios elementos agrupados en una misma celda. Toma por ejemplo en la tabla `starwars` que hemos utilizado a lo largo del curso.
...
Podemos hacer agrupaciones por mas de una variable, por ejemplo po planeta de origen y sexo:
...
Importa ambas bases de datos y en una sola cadena gráfica los valores de expresión de cada replica de cada tratamiento de los genes que estén involucradas en procesos catabolic `catabolic process` tal como se muestra en la imagen.
