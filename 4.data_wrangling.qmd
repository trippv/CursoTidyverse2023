...
### Filtrar variables con un patrón
...
Abre la tabla `palmer_penguis.csv` que se encuentra en el directorio de *databases* y utilizando pipies genera los siguientes objetos:
...
2. Una tabla con solamente los individuos hembras de culquier especie y que solamente contenta las columnas de _especie_, _isla_ y peso corporal **en Kg**.
...
Para hacer un resumen (_summary_) de una determinada variable en los datos podemos utilizar la función `summarise()` que a su vez nos permite aplicar una función (mean, sd, min, max) a una variable.
...
:::{.callout-tip}
Por default, la función `count()` nombra _n_ al número de items de cada groupo. Eso se puede modificar agregando el parámetro `name`.

```{r}
starwars %>% 
  count(species, sort = TRUE, name = "otros")
```
