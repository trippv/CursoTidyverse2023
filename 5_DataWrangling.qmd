...
![](img/hexs/dplyr.png){fig-align="center" width="182"}
...
Nuestro pokémon tiene las siguientes caracteristicas:
...
#paso 2: seleccionar las columnas con select()
starwars_df <- select(starwars, 
                      name, height, mass, sex, gender)

# paso 3: renombrar las columnas con rename()
starwars_df <- rename(starwars_df, 
                      nombre = name,
                      altura = height, 
                      masa = mass,
                      sexo = sex,
...
#paso 4: crear la relacion masa/altura
starwars_df <- mutate(starwars_df,
                      masa_altura = masa/altura)

#paso 5: filtrar los indiviuos con relacion masa/altura > 1
starwars_df <- filter(starwars_df, 
                      masa_altura >1 )
```
:::
...
attacks <- read_csv("data/attacks.csv")
...
![Allison Horst (\@allison_horst)](img/naniar.png){width="644"}
